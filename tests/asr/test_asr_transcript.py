...
def test_write_textgrid_uses_hash_for_silence_gaps(tmp_path: Path) -> None:
    transcript = Transcript(
        segments=[
            TranscriptSegment(
                start_time=0.5,
                end_time=1.0,
                text="bonjour",
                words=[WordTimestamp(word="bonjour", start_time=0.5, end_time=1.0, confidence=0.9)],
            )
        ]
    )
    path = tmp_path / "transcript_silence.TextGrid"
...
def test_align_transcript_to_ipus_normalizes_case_punctuation_and_apostrophes() -> None:
    transcript = Transcript(
        segments=[
            TranscriptSegment(
                start_time=0.0,
                ...
                text="ignored",
                words=[
                    WordTimestamp(word="L'", start_time=0.0, end_time=0.2, confidence=0.9),
                    WordTimestamp(word=" Homme", start_time=0.2, end_time=0.6, confidence=0.9),
                    WordTimestamp(word="...", start_time=0.6, end_time=0.7, confidence=0.9),
                    WordTimestamp(word="BONJOUR?", start_time=0.7, end_time=1.0, confidence=0.9),
                ],
            )
        ]
    )
    ...
    assert [word.word for word in aligned.segments[0].words] == ["l'", "homme", "bonjour"]
    assert aligned.segments[0].text == "l'homme bonjour"
